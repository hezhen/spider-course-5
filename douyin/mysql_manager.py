...
class MysqlManager:
    ...
    def dequeue_batch_urls(self, size):
        con = self.cnxpool.get_connection()
        cursor = con.cursor(dictionary=True)
        try:
            const_id = "%.9f" % time.time()
            ...
            print('dequeue_batch_urls() ' + err.msg)
            return None
        finally:
            cursor.close()
            con.close()
    ...
    def finish_url(self, index):
        ...
        try:
            ...
            update_query = ("UPDATE urls SET `status`='done' WHERE `id`=%d") % (index)
            cursor.execute(update_query)
            con.commit()
        except mysql.connector.Error as err:
...
if __name__ == "__main__":
    ...
    #     mysql_mgr.enqueue_url('https://www.douyin.com/all-{}'.format(i))
